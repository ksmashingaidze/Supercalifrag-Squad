...
public class MainMenuScript : MonoBehaviour
{
    ...
    public int gameplayedbefore; //Declare a new integer to keep track of whether the player has played the game before. On the first running of the game, the player preferences will be cleared.
    ...
    void Awake()
    {
        //Cursor.SetCursor(cursortexture, hotspot, cursormode); //Set the cursor texture
        gameplayedbefore = PlayerPrefs.GetInt("gameplayedbefore", 0); //Check if the player has played the game before. '0' for no, '1' for yes
        if (gameplayedbefore == 0) //If the game is being run for the first time
        {
            PlayerPrefs.DeleteAll(); //Delete all the player preferences
            gameplayedbefore = 1; //Update the relevant variable to reflect that the player has run the game for the first time
            PlayerPrefs.SetInt("gameplayedbefore", 1); //Update the player preferences to reflect that the player has run the game for the first time

        }
        //Else do nothing. Run the game as usual.
